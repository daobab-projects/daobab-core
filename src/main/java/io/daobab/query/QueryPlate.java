...
public final class QueryPlate extends Query<Entity, QueryPlate> implements QueryExpressionProvider<Entity, QueryPlate>, PlateProvider, QueryJoin<QueryPlate> {
    ...
    public QueryPlate(QueryTarget target, Map<String, Object> remote) {
    ...
    private QueryPlate andColumn(Column<?, ?, ?> columndao) {
    ...
    public Plates findMany() {
        return getTarget().readPlateList(modifyQuery());
    }

    @Override
    public Optional<Plate> findFirst() {
        return Optional.ofNullable(getTarget().readPlate(modifyQuery()));
    }
