...
public interface FunctionWhispererH2 {
    ...
    default <E extends Entity, F, R extends EntityRelation> ColumnFunction<E, F, R, F> sum(ColumnOrQuery... columns) {
        if (columns == null) throw new MandatoryFunctionParameter(DictFunctionH2.SUM);
        return new ManyArgumentsFunction<>(DictFunctionH2.SUM, "+", columns);
    }

    @SuppressWarnings("unchecked")
    default <E extends Entity, F, R extends EntityRelation> ColumnFunction<E, F, R, F> sub(Column... columns) {
        if (columns == null) throw new MandatoryFunctionParameter(DictFunctionH2.SUB);
        return new ManyArgumentsFunction<>(DictFunctionH2.SUB, "-", columns);
    }
