...
/**
 * Standard converter for enum columns stored by an integer code, where the enum implements {@link IntBasedEnum}:
 * the database {@code Integer} is matched against the constants' {@code getValue()} on reading and written back
 * as that code.
 *
 * @param <E> the enum type, carrying an integer code
...
public class StandardTypeConverterEnumIntBased<E extends Enum & IntBasedEnum> extends TypeConverterIntegerBased<E> {
    ...
    /**
     * Resolves the integer code to the constant whose {@code getValue()} matches.
     *
    ...
    /** Renders the constant's integer code as text, or {@code null}. */
    ...
    /** Binds the constant's integer code, or {@code null}. */
    @SuppressWarnings("unchecked")
    @Override
