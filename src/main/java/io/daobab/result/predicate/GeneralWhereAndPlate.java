...
public class GeneralWhereAndPlate<P extends Plate> implements WherePredicate<P> {
    ...
    private Predicate<Object> matchPredicate(Where wrapper, int i) {
        ...
        if (where != null) {
            ...
            if (OR.equals(innerRelation)) {
                return new GeneralWhereOrPlate(wrval);
            }
            //for AND and NOT
            return new GeneralWhereAndPlate(wrval);
        }
        ...
        if (datetime) {
            ...
            if (valueFromWrapper instanceof Integer) {
                return matchIntegerComparationPredicate((Integer) valueFromWrapper, relation);
            }
    ...
    private Predicate matchIntegerComparationPredicate(Integer numericValueFromWrapper, Operator operator) {

        switch (operator) {
            case EQ:
                return new IntegerMatchEQ(numericValueFromWrapper);
            case NOT_EQ:
                return new IntegerMatchNotEQ(numericValueFromWrapper);
            case NOT_NULL:
                return new MatchNotNull();
            case IS_NULL:
                return new MatchIsNull();
            case GT:
                return new IntegerMatchGT(numericValueFromWrapper);
            case GTEQ:
                return new IntegerMatchGTEQ(numericValueFromWrapper);
            case LT:
                return new IntegerMatchLT(numericValueFromWrapper);
            case LTEQ:
                return new IntegerMatchLTEQ(numericValueFromWrapper);
            default:
                throw new UnhandledOperator(operator);
        }
